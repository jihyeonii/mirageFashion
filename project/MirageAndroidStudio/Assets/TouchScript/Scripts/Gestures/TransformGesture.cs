    ...
    public class TransformGesture : TransformGestureBase, ITransformGesture
    {
                                                         ...
                                                         ProjectionParams projectionParams)
        {
            ...
            if (isTransforming)
            {
                ...
                {
#if UNITY_IOS
                    GameManager.instancs.character.transfom.Find("collider").transform.position = new Vector3((oldScreenPos.x - newScreenPos.x), (newScreenPos.y - oldScreenPos.y), 0);
                    return projectionParams.ProjectTo(new Vector2(oldScreenPos.x, newScreenPos.y), TransformPlane) -
                       projectionParams.ProjectTo(new Vector2(newScreenPos.x, oldScreenPos.y), TransformPlane);
#else
                    return projectionParams.ProjectTo(new Vector2(newScreenPos.x, oldScreenPos.y), TransformPlane) -
                       projectionParams.ProjectTo(new Vector2(oldScreenPos.x, newScreenPos.y), TransformPlane);
#endif
                }
                else
                {
                                                         ...
                                                         Vector2 newScreenPos1, Vector2 newScreenPos2, float dR, float dS, ProjectionParams projectionParams)
        {
            if (isTransforming)
            {
                ...
                {
#if UNITY_IOS
                    GameManager.instancs.character.transfom.Find("collider").transform.position = new Vector3((oldScreenPos1.x - newScreenPos1.x), (newScreenPos1.y - oldScreenPos1.y), 0);
                    return projectionParams.ProjectTo(new Vector2(oldScreenPos1.x, newScreenPos1.y), TransformPlane) -
                        projectScaledRotated(new Vector2(newScreenPos1.x, oldScreenPos1.y), dR, dS, projectionParams);
#else

                    return projectionParams.ProjectTo(new Vector2(newScreenPos1.x, oldScreenPos1.y), TransformPlane) -
                        projectScaledRotated(new Vector2(oldScreenPos1.x, newScreenPos1.y), dR, dS, projectionParams);
#endif
                }
                else
                {
