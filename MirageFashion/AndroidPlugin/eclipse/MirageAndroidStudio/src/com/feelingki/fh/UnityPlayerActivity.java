...
//		File[] uriFiles = (new File(Environment.getExternalStorageDirectory() + "/DCIM/Mirage/").listFiles());
	...
	public void mediaScan(String name){
		if(Build.VERSION.SDK_INT <10){
			sendBroadcast(new Intent(Intent.ACTION_MEDIA_SCANNER_SCAN_FILE, Uri.parse(Environment.getExternalStorageDirectory()+ "/DCIM/Mirage/" + name)));
		}
		else
		{
//			sendBroadcast(new Intent(Intent.ACTION_MEDIA_MOUNTED, Uri.parse("file://"+Environment.getExternalStorageDirectory()+"/Mirage")));
			File file = new File(Environment.getExternalStorageDirectory()+ "/DCIM/Mirage/" + name);
			Log.d("=====", "version > 10");
